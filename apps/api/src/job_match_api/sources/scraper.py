...
def _dari_item(item: dict) -> ScraperJob:
...
def daftar_keyword(kata: str) -> None:
    ...
    try:
        ...
        raise ScraperError(f"Scraper menolak keyword (HTTP {e.response.status_code})") from e
    except httpx.RequestError as e:
        raise ScraperError(f"Tidak bisa menghubungi scraper: {type(e).__name__}") from e


def search(days: int = 7, limit: int = 100) -> list[ScraperJob]:
    ...
    try:
        ...
        raise ScraperError(f"Scraper menolak permintaan (HTTP {e.response.status_code})") from e
    except httpx.RequestError as e:
        raise ScraperError(f"Tidak bisa menghubungi scraper: {type(e).__name__}") from e
